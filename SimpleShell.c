...
// Josh Lewis, Project 1, 2/20/2018
...
{
	return execvp(path_to_exec, args) ? -1 : 0; // Modified for P5.1 test
}
...
void imtheparent(pid_t child_pid, int run_in_background)
{
	...
	//Old code:  wait(&child_return_val);
...
int main(int argc, char **argv)
{
    ...
    int cmd_counter = 1; // for P5.2 test
    char history_buffer[9][SHELL_BUFFER_SIZE]; // for P5.3 test
    int sub_shell_depth = 0; // variable for P5.6
	...
	/* The Shell runs in an infinite loop, processing input. */
        ...
        /* Check if we're executing previous command (P5.3) */
        if(buffer[0] == '!'){
        ...
        /* Update number of cmd_counter for P5.2 test and command history for P5.3 */
        if(buffer[0] != '\n'){
            if(cmd_counter <= 9){
		...
		for (exec_argc = 0, i = 0;
			...
			if (!isspace(buffer[i])) { // As long as entry is not a space...
				if (parser_state == STATE_SPACE)
					exec_argv[exec_argc++] = &buffer[i];
				parser_state = STATE_NON_SPACE;
			} else { // Make the current entry null (space)
				buffer[i] = '\0';
				parser_state = STATE_SPACE;
			}
		}
